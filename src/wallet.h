...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 OCASHCOIN at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zOCASH send
enum ZerocoinSpendStatus {
    ZOCASH_SPEND_OKAY = 0,                            // No error
    ZOCASH_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZOCASH_WALLET_LOCKED = 2,                         // Wallet was locked
    ZOCASH_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZOCASH_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZOCASH_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZOCASH_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZOCASH_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZOCASH_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZOCASH_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZOCASH_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZOCASH_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZOCASH_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZOCASH_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZOCASH_SPENT_USED_ZOCASH = 14,                      // Coin has already been spend
    ZOCASH_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZOCASH_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void Z0cashBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZOCASHOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzocashAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzOCASHTracker> zocashTracker;
    ...
    void setZWallet(CzOCASHWallet* zwallet)
    {
        zwalletMain = zwallet;
        zocashTracker = std::unique_ptr<CzOCASHTracker>(new CzOCASHTracker(strWalletFile));
    }

    CzOCASHWallet* getZWallet() { return zwalletMain; }
    ...
    void setZ0cashAutoBackups(bool fEnabled)
    {
    ...
    /** zOCASH reset */
    boost::signals2::signal<void()> NotifyzOCASHReset;
