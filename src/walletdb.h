...
public:
    ...
    bool WriteZOCASHSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZOCASHSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZOCASHSeed_deprecated(uint256& seed);
    bool EraseZOCASHSeed();
    bool EraseZOCASHSeed_deprecated();

    bool WriteZOCASHCount(const uint32_t& nCount);
    bool ReadZOCASHCount(uint32_t& nCount);
