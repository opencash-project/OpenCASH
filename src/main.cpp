...
#if defined(NDEBUG)
#error "OCASHCOIN cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uocash) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zOCASH spend with serial %s is already in block %d\n", __func__,
...
{
    //Check to see if the zOCASH is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zOCASH spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zOCASH without the correct spend type. txid=%s", __func__,
                tx.GetHash().GetHex());
        }
    }    
...

    {
        ...
        if (tx.IsZerocoinSpend()) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zOCASH spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                             tx.GetHash().GetHex()),
                        REJECT_INVALID, "bad-txns-invalid-zocash");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zOCASH mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 1) {
        ...
        nSubsidy = 5500 * COIN;
    } else if (nHeight < 120000 && nHeight >= 20000) {
        nSubsidy = 4500 * COIN;
    } else if (nHeight < 320000 && nHeight >= 120000) {
        nSubsidy = 3500 * COIN;
    } else if (nHeight < 500000 && nHeight >= 320000) {
        nSubsidy = 2500 * COIN;
    } else if (nHeight < 700000 && nHeight >= 500000) {
        nSubsidy = 2000 * COIN;
    } else if (nHeight < 900000 && nHeight >= 700000) {
        nSubsidy = 3300 * COIN;
    } else if (nHeight < 1550000 && nHeight >= 900000) {
        nSubsidy = 3500 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZ0cashStake)
{
    if(!isZ0cashStake){
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from OCASHCOIN
...
{
    RenameThread("opencash-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZOCASHMinted()
{
...
void RecalculateZOCASHSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zOCASH supply
...
bool RecalculateOCASHCOINSupply(int nHeightStart)
{
...
bool UpdateZOCASHSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
    ...
    CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    //Track zOCASH money supply in the block index
    if (!UpdateZOCASHSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zOCASH supply for block=%s height=%d", __func__, block.GetHash().GetHex(), pindex->nHeight), REJECT_INVALID);
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert OCASHCOIN to zOCASH
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zOCASH spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn& txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zOCASH serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    ...
    //     return error("%s: zOCASH stake block is less than allowed start height", __func__);

    if (CZ0cashStake* zOCASH = dynamic_cast<CZ0cashStake*>(stake)) {
        CBlockIndex* pindexFrom = zOCASH->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zOCASH stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zOCASH stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zOCASH->GetValue()));
        if (nChecksum200 != zOCASH->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zOCASH->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZOCASH() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zOCASH fails context checks", __func__));
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> ocashInputs;
        std::vector<CTxIn> zOCASHInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.scriptSig.IsZerocoinSpend()){
                zOCASHInputs.push_back(stakeIn);
            }else{
                ocashInputs.push_back(stakeIn);
            }
        }
        const bool hasOCASHInputs = !ocashInputs.empty();
        const bool hasZOCASHInputs = !zOCASHInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasOCASHInputs)
                    ...
                    for (const CTxIn& ocashIn : ocashInputs){
                        if(ocashIn.prevout == in.prevout){
        ...
        if (isBlockFromFork) {
            ...
            do {
                ...
                for (const CTransaction& t : bl.vtx) {
                    for (const CTxIn& in: t.vin) {
                        ...
                        for (const CTxIn& stakeIn : ocashInputs) {
            ...
            // Now that this loop if completed. Check if we have zOCASH inputs.
            if(hasZOCASHInputs){
                for (const CTxIn& z0cashInput : zOCASHInputs) {
                    CoinSpend spend = TxInToZerocoinSpend(z0cashInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, spend, pindex, 0))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zocash");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zOCASH mints and %d zOCASH spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // OCASHCOIN: We use certain sporks during IBD, so check to see if they are
        ...
        if(nLocalServices & NODE_BLOOM_LIGHT_ZC) {
            try {
                ...
                if (!GetAccumulatorValue(height, den, bnAccValue)) {
                    LogPrint("zocash", "peer misbehaving for request an invalid acc checkpoint \n", __func__);
                    Misbehaving(pfrom->GetId(), 50);
                } else {
            ...
            try {
                ...
                if (gen.isValid(chainActive.Height())) {
                    if (!lightWorker.addWitWork(gen)) {
                        LogPrint("zocash", "%s : add genwit request failed \n", __func__);
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
