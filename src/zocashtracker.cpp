...
CzOCASHTracker::CzOCASHTracker(std::string strWalletFile)
{
...
CzOCASHTracker::~CzOCASHTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzOCASHTracker::Init()
{
...
bool CzOCASHTracker::Archive(CMintMeta& meta)
{
...
bool CzOCASHTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzOCASHTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzOCASHTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzOCASHTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzOCASHTracker::GetSerialHashes()
{
...
CAmount CzOCASHTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzOCASHTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzOCASHTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzOCASHTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzOCASHTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzOCASHTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzOCASHTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzOCASHTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzOCASHTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzOCASHTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzOCASHTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzOCASHTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzOCASHTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzOCASHTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzOCASHTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzOCASHTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzOCASHTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzocash from DB\n", __func__, listDeterministicDB.size());
    }
