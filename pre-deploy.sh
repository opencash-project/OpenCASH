...
if [[ $HOST = "x86_64-apple-darwin11" ]]; then
    find $TRAVIS_BUILD_DIR -type f | grep -i opencash-qt.dmg$ | xargs -i cp {} $OUTDIR/bin
    find $TRAVIS_BUILD_DIR -type f | grep -i opencash-core.dmg$ | xargs -i cp {} $OUTDIR/bin
else
    cp $TRAVIS_BUILD_DIR/src/qt/opencash-qt $OUTDIR/bin/ || cp $TRAVIS_BUILD_DIR/src/qt/opencash-qt.exe $OUTDIR/bin/ || echo "no QT Wallet"
    cp $TRAVIS_BUILD_DIR/src/opencashd $OUTDIR/bin/ || cp $TRAVIS_BUILD_DIR/src/opencash.exe $OUTDIR/bin/
    cp $TRAVIS_BUILD_DIR/src/opencash-cli $OUTDIR/bin/ || cp $TRAVIS_BUILD_DIR/src/opencash-cli.exe $OUTDIR/bin/
