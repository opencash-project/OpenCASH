...
#include "ui_zocashcontroldialog.h"
...
std::set<std::string> Z0cashControlDialog::setSelectedMints;
std::set<CMintMeta> Z0cashControlDialog::setMints;

bool CZ0cashControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == Z0cashControlDialog::COLUMN_DENOMINATION || column == Z0cashControlDialog::COLUMN_VERSION || column == Z0cashControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


Z0cashControlDialog::Z0cashControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::Z0cashControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
Z0cashControlDialog::~Z0cashControlDialog()
{
    delete ui;
}

void Z0cashControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void Z0cashControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZ0cashControlWidgetItem* itemDenom(new CZ0cashControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZ0cashControlWidgetItem *itemMint = new CZ0cashControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
...
void Z0cashControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void Z0cashControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZ0cash_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZ0cashControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> Z0cashControlDialog::GetSelectedMints()
{
...
void Z0cashControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
