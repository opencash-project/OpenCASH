...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zocashTracker = unique_ptr<CzOCASHTracker>(new CzOCASHTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZOCASH_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZOCASH_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
