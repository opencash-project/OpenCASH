...
#ifndef OCASHCOIN_ZOCASHWALLET_H
#define OCASHCOIN_ZOCASHWALLET_H
...
public:
    CzOCASHWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZOCASH(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZOCASH(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //OCASHCOIN_ZOCASHWALLET_H
