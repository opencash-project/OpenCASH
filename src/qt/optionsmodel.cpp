...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::OCASH);
    ...
    if (!settings.contains("nAnonymizeOpenCASHAmount"))
        settings.setValue("nAnonymizeOpenCASHAmount", 1000);

    nAnonymizeOpenCASHAmount = settings.value("nAnonymizeOpenCASHAmount").toLongLong();
...
// Wallet
    ...
    if (!settings.contains("addrProxy"))
        settings.setValue("addrProxy", "127.0.0.1:43799");
    ...
    if (settings.contains("nAnonymizeOpenCASHAmount"))
        SoftSetArg("-anonymizeopencashamount", settings.value("nAnonymizeOpenCASHAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in opencash.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeOpenCASHAmount:
            return QVariant(nAnonymizeOpenCASHAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeOpenCASHAmount:
            nAnonymizeOpenCASHAmount = value.toInt();
            settings.setValue("nAnonymizeOpenCASHAmount", nAnonymizeOpenCASHAmount);
            emit anonymizeOpenCASHAmountChanged(nAnonymizeOpenCASHAmount);
            break;
        case CoinControlFeatures:
