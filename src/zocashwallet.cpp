...
CzOCASHWallet::CzOCASHWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zocash seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZOCASHSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZOCASHSeed_deprecated()) {
                    LogPrintf("%s: Updated zOCASH seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zocash seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zocash wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzOCASHWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZOCASHCount(nCountLastUsed);
    else if (!walletdb.ReadZOCASHCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzOCASHWallet::Lock()
{
    seedMaster = 0;
}

void CzOCASHWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzOCASHWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZOCASH(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzOCASHWallet::LoadMintPoolFromDB()
{
...
void CzOCASHWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzOCASHWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzOCASHWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zocashTracker->HasPubcoinHash(pMint.first)) {
...
bool CzOCASHWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZOCASH(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zocashTracker which also adds to database
    pwalletMain->zocashTracker->Add(dMint, true);
...
void CzOCASHWallet::SeedToZOCASH(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzOCASHWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzOCASHWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZOCASHCount(nCountLastUsed);
...
void CzOCASHWallet::GenerateDeterministicZOCASH(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzOCASHWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzOCASHWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
