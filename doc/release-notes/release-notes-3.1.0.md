OpenCASH Core version *3.1.0* is now available from:  <https://github.com/opencash-project/opencash/releases>

This is a new major version release, including various bug fixes and performance improvements, as well as updated translations.

Please report bugs using the issue tracker at github: <https://github.com/opencash-project/opencash/issues>


Mandatory Update
==============

OpenCASH Core v3.1.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will have a grace period of one week to update their clients before enforcement of this update is enabled.
...
zOCASH Staking is here! zOCASH staking will be activated on the morning of the 8th of May 2018. With the release of zOCASH staking, there are effectively 2 versions of zOCASH, zOCASH minted on the 3.0.6 OCASHCOIN wallet or lower, and zOCASH minted on OCASHCOIN wallet version or higher. New features in this release will require the use of zOCASH v2, zOCASH minted on this wallet release 3.1.0 or later. If you currently hold zOCASH v1 and wish to take advantage of zOCASH staking and deterministic zOCASH, you will need to spend the zOCASH v1 to yourself and remint zOCASH v2.
Note: To find your zOCASH version, click the privacy tab, then the zOCASH Control button then expand the arrows next to the desired denomination.


### Deterministic zOCASH Seed Keys

zOCASH is now associated with a deterministic seed key. With this seed key, users are able to securely backup their zOCASH outside of the wallet that the zOCASH had been minted on. zOCASH can also be transferred from wallet to wallet without the need of transferring the wallet data file.


### Updated zOCASH minting

zOCASH minting now only requires 1 further mint (down from 2) to mature. zOCASH mints still require 20 confirmations.  Mints also require that the 'second' mint is at least two checkpoints deep in the chain (this was already the case, but the logic was not as precise).


### zOCASH Search

Users will now have the ability to search the blockchain for a specific serial # to see if a zOCASH denomination has been spent or not.



OCASHCOIN/zOCASH Staking and Masternode Rewards
--------------

### OCASHCOIN, zOCASH and Masternode Payment Schedule

To encourage the use of zOCASH and increase the OCASHCOIN zerocoin anonymity set, the OCASHCOIN payment schedule has been changed to the following:

If a user staking zOCASH wins the reward for their block, the following zOCASH reward will be: 
- 3 zOCASH (3 x 1 denominations) rewarded to the staker, 2 OCASHCOIN rewarded to the masternode owner and 1 OCASHCOIN available for the budget. This is a total block reward of 6 OCASHCOIN, up from 5.

If a user staking OCASHCOIN wins the reward, the following amounts will be rewarded: 
- 2 OCASHCOIN to the OCASHCOIN staker, 3 OCASHCOIN to the Masternode owner and 1 OCASHCOIN available for the budget. This is a total block reward of 6 OCASHCOIN, up from 5.


### Return change to sender when minting zOCASH

Previously, zOCASH minting would send any change to a newly generated "change address". This has caused confusion among some users, and in some cases insufficient backups of the wallet. The wallet will now find the contributing address which contained the most OCASHCOIN and return the change from a zOCASH mint to that address.
...
The visual layout of the OCASHCOIN Qt wallet has undergone a near-complete overhaul.
...
-	Enable and disable the auto zOCASH minting feature. This is enabled by default and the enablezeromint=0 setting in the opencash.conf file will overwrite the GUI option.
-	The percentage of autominted zOCASH can now be set from 1 to 100, changed from 10 – 100.
-	The stake split threshold can now be set VIA the wallet options. This setting is an advanced feature for those wishing to remain staking regular OCASHCOIN.
...
### Summary
 ...
 The OCASHCOIN wallet can now have user selected directories for automatic backups of the wallet data file (wallet.dat). This can be set by adding the following lines to the opencash.conf file, found in the OCASHCOIN data directory.
- backuppath = <directory / full path>
- zocashbackuppath = <directory / full path>
...
* If zOCASH backup, auto generated name is `wallet-autozocashbackup.dat-<year>-<month>-<day>-<hour>-<minute>-<second>`
...
OCASHCOIN Daemon & Client (RPC Changes)
--------------

### RPC Ban Management

The OCASHCOIN client peer bans now have additional RPC commands to manage peers. Peers can be banned and unbanned at will without the need to restart the wallet client. No changes have been made to the conditions resulting in automatic peer bans. New RPC commands: `setban`, `listbanned`, `clearbanned`, and `disconnectnode`
...
This allows running opencashd without having to do any manual configuration.
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the OCASHCOIN support group, which often times sees users say "I didn't spend that zOCASH". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
As voted on by the OCASHCOIN masternodes, the monthly budget available to be utilised has been increased to 42,000 OCASHCOIN / month. This OCASHCOIN only has the opportunity to be raised once per month (paid to winning proposals) with any unused OCASHCOIN not created by the blockchain.

### Budget Finalization Fee

The OCASHCOIN finalization fee for successful proposals has now been reduced, this fee is now 5 OCASHCOIN down from 50 OCASHCOIN. The total fee outlay for a successful proposal is now a total of 55 OCASHCOIN.
...
-	Enforced v1 zOCASH spends to require a security level of 100
-	Updates to zOCASH spends to avoid segfaults
-	Updates to configuration will now reflect on the privacy tab
-	Fixed a  bug that would not start masternodes from the OCASHCOIN-Qt masternodes tab
-	Updated OCASHCOIN-Qt tooltips
...
Zerocoins, or zOCASH, are now deterministically generated using a unique 256 bit seed. Each wallet will generate a new seed on its first run. The deterministic seed is used to generate a string of zOCASH that can be recalculated at any time using the seed. Deterministic zOCASH allows for users to backup all of their future zOCASH by simply recording their seed and keeping it in a safe place (similar to backing up a private key for OCASHCOIN). The zOCASH seed needs to remain in the wallet in order to spend the zOCASH after it is generated, if the seed is changed then the coins will not be spendable because the wallet will not have the ability to regenerate all of the private zOCASH data from the seed. It is important that users record & backup their seed after their first run of the wallet. If the wallet is locked during the first run, then the seed will be generated the first time the wallet is unlocked.

Zerocoin Modulus
--------------

OCASHCOIN's zerocoin implementation used the same code from the ZCoin project to import the modulus use for the zerocoin protocol. The chosen modulus is the 2048 bit RSA number created for the RSA factoring challenge. The ZCoin project's implementation (which OCASHCOIN used) improperly imported the modulus into the code. This flaw was discovered by user GOAT from the [Civitas Project](https://github.com/eastcoastcrypto/Civitas/), and reported to OCASHCOIN using the bug bounty program. The modulus is now correctly imported and OCASHCOIN's accumulators have been changed to use the new proper modulus.
...
### Core Features
 - #582 `cb1632520` [Core] zOCASH v2: zPoS dzOCASH ezOCASH external backup and budget fixes (too many to list)
...
### GUI
 - #572 `d9b23fe60` [Qt] Refresh zOCASH balance after resetting mints or spends (warrows)
 ...
 - #506 `ae72bf4e2` [Qt] Autoscroll to end of zOCASH status output (Mrs-X)
 ...
 - #467 `7099831bd` [Qt] Clear PrivacyDialog "z0cash Selected" labels after sending. (presstab)
...
### RPC/REST
 ...
 - #543 `e4522ff07` [RPC] Segfault opencash-cli getinfo while loading block index (Mrs-X)
...
### Wallet
 - #570 `8c8350b59` [Wallet] Add a check on zOCASH spend to avoid a segfault (warrows)
 ...
 - #448 `222ef6e6b` [Wallet] Return change to sender when minting zOCASH. (presstab)
 - #445 `fc570fc1e` [Wallet] Only require 1 mint to be added before spending zOCASH. (presstab)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/opencash-project-translations/), the QA team during Testing and the Node hosts supporting our Testnet.
