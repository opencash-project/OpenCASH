...
#include "lightzocashthread.h"
#include "main.h"

/****** Thread ********/
void CLightWorker::ThreadLightZOCASHSimplified() {
    RenameThread("ocashx-light-thread");
    ...
    while (true) {
        try {
            ...
            LogPrintf("%s pop work for %s \n\n", "ocashx-light-thread", genWit.toString());
            ...
            if (!pIndex) {
                ...
                LogPrintf("%s calculating work for %s \n\n", "ocash-light-thread", genWit.toString());
                ...
                if (blockHeight >= Params().Zerocoin_Block_V2_Start()) {
                    ...
                    try {
                        ...
                        LogPrintStr(std::string("ThreadLightZOCASHSimplified: ") + e.message + "\n");
                    ...
                    if (!res) {
                        ...
                        ss << heightStop;
                        if (genWit.getPfrom()) {
                            LogPrintf("%s pushing message to %s \n", "ocash-light-thread", genWit.getPfrom()->addrName);
            ...
            PrintExceptionContinue(&e, "lightzocashthread");
            break;
        }
    }
...
void CLightWorker::rejectWork(CGenWit& wit, int blockHeight, uint32_t errorNumber) {
    if (wit.getStartingHeight() == blockHeight){
        LogPrintf("%s rejecting work %s , error code: %s\n", "ocash-light-thread", wit.toString(), errorNumber);
        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
