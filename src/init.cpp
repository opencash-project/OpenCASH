...
#include "config/opencash-config.h"
#endif
...
CzOCASHWallet* zwalletMain = NULL;
...
void Shutdown()
{
    ...
    if (nLocalServices == NODE_BLOOM_LIGHT_ZC) {
        lightWorker.StopLightZocashThread();
    }
#ifdef ENABLE_WALLET
    delete pwalletMain;
    pwalletMain = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "opencash.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "opencashd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the OCASHCOIN and zOCASH money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 43799, 10500));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in OCASHCOIN/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in OCASHCOIN/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, opencash, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in OCASHCOIN/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all OCASHCOIN specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-ocashstake=<n>", strprintf(_("Enable or disable staking functionality for OCASHCOIN inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zocashstake=<n>", strprintf(_("Enable or disable staking functionality for zOCASH inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:43799"));
    ...
    strUsage += HelpMessageOpt("-backupzocash=<n>", strprintf(_("Enable automatic wallet backups triggered after each zOCASH minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zocashbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zOCASH backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizeopencashamount=<n>     " + strprintf(_("Keep N OCASHCOIN anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 43800, 11000));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2019-%i The OCASH Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2020-%i The OpenCASH Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that OCASHCOIN is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. OpenCASH Core is shutting down."));
...
#endif
    // Make sure only a single OCASHCOIN process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. OpenCASH Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("OCASHCOIN version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    if (proxyArg != "" && proxyArg != "0") {
        CService proxyAddr;
        if (!Lookup(proxyArg.c_str(), proxyAddr, 43799, fNameLookup)) {
    ...
    if (onionArg != "") {
        if (onionArg == "0") { // Handle -noonion/-onion=0
            ...
            CService onionProxy;
            if (!Lookup(onionArg.c_str(), onionProxy, 43799, fNameLookup)) {
    ...
    //OCASHCOIN: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //OCASHCOIN specific: zerocoin and spork DB's
                ...
                // OCASHCOIN: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    //if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZOCASHMinted();
                        RecalculateZOCASHSpent();
                    //}
                    RecalculateOCASHCOINSupply(1);
                }
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    //if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        ...
                        // OCASHCOIN: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of OpenCASH Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart OpenCASH Core to complete") << "\n";
        ...
        zwalletMain = new CzOCASHWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zOCASHWallet
        uiInterface.InitMessage(_("Syncing zOCASH wallet..."));

        pwalletMain->InitAutoConvertAddresses();

        bool fEnableZ0cashBackups = GetBoolArg("-backupzocash", true);
        pwalletMain->setZ0cashAutoBackups(fEnableZ0cashBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zocashTracker->Init();
...
//    }
//
//    nAnonymizeOpenCASHAmount = GetArg("-anonymizeopencashamount", 0);
//    if (nAnonymizeOpenCASHAmount > 999999) nAnonymizeOpenCASHAmount = 999999;
//    if (nAnonymizeOpenCASHAmount < 2) nAnonymizeOpenCASHAmount = 2;
    ...
    LogPrintf("Anonymize OCASHCOIN Amount %d\n", nAnonymizeOpenCASHAmount);
    ...
    /* Denominations
       ...
       For example:
       1OCASHCOIN+1000 == (.1OCASHCOIN+100)*10
       10OCASHCOIN+10000 == (1OCASHCOIN+1000)*10
    */
    ...
    */
    ...
    if (nLocalServices & NODE_BLOOM_LIGHT_ZC) {
        ...
        lightWorker.StartLightZocashThread(threadGroup);
    }
